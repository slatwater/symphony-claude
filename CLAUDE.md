...
自动化编码 Agent 编排服务：轮询 GitHub Issues → 创建隔离工作区 → 调度 Claude Code Agent 自动完成开发任务。
...
| `elixir/lib/symphony_elixir/claude_code/` | Claude Code 集成（client） |
| `elixir/lib/symphony_elixir/codex/` | Codex 集成（原有，备用后端） |
| `elixir/lib/symphony_elixir/github/` | GitHub Issues REST API 客户端 |
...
- Tracker kind: `github`（通过 GitHub REST API 轮询 Issues，用 labels 管理工作流状态）
...
- Agent 通过 `gh` CLI 直接与 GitHub 交互（无需 MCP）
