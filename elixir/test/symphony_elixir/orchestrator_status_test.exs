defmodule SymphonyElixir.OrchestratorStatusTest do
  ...
  test "status dashboard renders github repo link in header" do
    snapshot_data =
      {:ok,
       %{
         running: [],
    ...
    assert rendered =~ "https://github.com/testowner/testrepo/issues"
  ...
  test "status dashboard humanizes full codex app-server event set" do
    event_cases = [
      ...
      {"item/tool/call", %{"params" => %{"tool" => "github_api"}}, "dynamic tool call requested (github_api)"},
  ...
  test "status dashboard humanizes dynamic tool wrapper events" do
    completed = %{
      ...
      message: %{
        payload: %{"method" => "item/tool/call", "params" => %{"name" => "github_api"}}
      }
    }
    ...
    failed = %{
      ...
      message: %{
        payload: %{"method" => "item/tool/call", "params" => %{"tool" => "github_api"}}
      }
    }
    ...
    assert StatusDashboard.humanize_codex_message(completed) =~
             "dynamic tool call completed (github_api)"

    assert StatusDashboard.humanize_codex_message(failed) =~
             "dynamic tool call failed (github_api)"
  ...
  test "status dashboard enriches wrapper reasoning and message streaming events with payload context" do
    reasoning_message = %{
      ...
      message: %{
        ...
        "params" => %{
          "msg" => %{
            "payload" => %{"summaryText" => "compare retry paths for GitHub polling"}
          }
        }
      }
    }
    ...
    assert StatusDashboard.humanize_codex_message(reasoning_message) =~
             "reasoning update: compare retry paths for GitHub polling"
