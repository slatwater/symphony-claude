...
1. Polls GitHub Issues for candidate work (label-based state management)
2. Creates an isolated workspace per issue
3. Launches Claude Code agent inside the workspace
4. Sends a workflow prompt to the agent
5. Keeps the agent working on the issue until the work is done

If a claimed issue moves to a terminal state (`done`, `cancelled`),
...
2. Create a GitHub personal access token with repo scope and set it as the `GITHUB_TOKEN`
   environment variable.
3. Copy this directory's `WORKFLOW.md` to your repo.
4. Customize the copied `WORKFLOW.md` file for your project.
   - Set `tracker.repo` to your `owner/repo` format.
   - Create GitHub labels matching your workflow states (e.g. `todo`, `in-progress`, `done`).
